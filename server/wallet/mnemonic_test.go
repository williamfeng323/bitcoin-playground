...
func TestInitWordList(t *testing.T) {
	initWordList()
	assert.NotNil(t, wordMap)
	assert.Equal(t, 2049, len(wordMap))
}

func TestGenerateEntropy(t *testing.T) {
	ent, err := GenerateEntropy(33)
	assert.Error(t, err)
	assert.Empty(t, ent)

	ent, err = GenerateEntropy(128)
	s1 := string(ent.entropy)
	assert.Nil(t, err)
	assert.NotEmpty(t, ent)

	ent, err = GenerateEntropy(128)
	s2 := string(ent.entropy)
	assert.Nil(t, err)
	assert.NotEmpty(t, ent)
	assert.NotEqual(t, s1, s2)
}

func TestEntropy_CalCheckSum(t *testing.T) {
	bitSize := 192
	ent, err := GenerateEntropy(bitSize)
	assert.Nil(t, err)
...
func TestGenerateMnemonic(t *testing.T) {
	for _, vector := range vectors {
		...
		assert.Equal(t, mnemonic, vector[1])
	}
}
...
func TestMnemonicToSeed(t *testing.T) {
	for _, vector := range vectors {
		...
		assert.Equal(t, hex.EncodeToString(seed), vector[2])
	}
}
